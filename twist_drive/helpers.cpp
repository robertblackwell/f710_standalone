...
const double max_angular_vel_rads__per_sec = (2.0 * max_wheel_speed_mm_per_sec)/axle_length_mm;
...
void sticks_to_twist(int f710_left, int f710_right, double*  linear_vel, double* angular_vel)
{

}
void twist_targets(int left, int right)
{
    double vel_linear_target = right_stick_to_linear_vel(right);
    double angular_target = left_stick_to_angular_vel(left);
    double radius_target = vel_linear_target / angular_target;
    double v_test_left = angular_target * (radius_target + (axle_length_mm / 2.0));
    double v_test_right = angular_target * (radius_target - (axle_length_mm / 2.0));
}
double right_stick_to_linear_vel(int f710_right)
{
    if(f710_right == 0) {
        return 0.0;
    }
    if(f710_right > 0) {
        return max_wheel_speed_mm_per_sec * ((double)f710_right / (double)F710_MAX_FORWARD);
    }
    return max_wheel_speed_mm_per_sec * ((double)f710_right / (double)F710_MAX_REVERSE);
}
double left_stick_to_angular_vel(int f710_left)
{
    if(f710_left == 0) {
        return 0.0;
    }
    if(f710_left > 0) {
        return max_angular_vel_rads__per_sec * ((double)f710_left) / ((double)F710_MAX_FORWARD);
    }
    return max_angular_vel_rads__per_sec * ((double)f710_left) / ((double)F710_MAX_REVERSE);
}
/**
...
{
    double l, r;
    if(twist_drive::is_max_forward(f710_left, f710_right)) {
        r = 95.0;
        l = 100.0;
    } else if(twist_drive::is_max_reverse(f710_left, f710_right)) {
        l = -100.0;
        r = -95.0;
    } else {
